...
use API\Model\Map\TeacherTableMap;
...
abstract class Teacher implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\API\\Model\\Map\\TeacherTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildAssignment[] Collection to store aggregation of ChildAssignment objects.
     */
     ...
     * @var ObjectCollection|ChildAssignment[]
     */
    protected $assignmentsScheduledForDeletion = null;

    /**
     * Initializes internal state of API\Model\Base\Teacher object.
     */
     ...
     * @return $this|\API\Model\Teacher The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : TeacherTableMap::translateFieldName('Email', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\API\\Model\\Teacher'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->assignmentsScheduledForDeletion !== null) {
                if (!$this->assignmentsScheduledForDeletion->isEmpty()) {
                    foreach ($this->assignmentsScheduledForDeletion as $assignment) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collAssignments) {
                ...
                $result[$key] = $this->collAssignments->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\API\Model\Teacher The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \API\Model\Teacher (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \API\Model\Teacher Clone of current object.
     * @throws PropelException
     */
    ...
    public function initAssignments($overrideExisting = true)
    {
        ...
        $collectionClassName = AssignmentTableMap::getTableMap()->getCollectionClassName();

        $this->collAssignments = new $collectionClassName;
        $this->collAssignments->setModel('\API\Model\Assignment');
    }

    /**
     * Gets an array of ChildAssignment objects which contain a foreign key that references this object.
     *
     ...
     * @return ObjectCollection|ChildAssignment[] List of ChildAssignment objects
     * @throws PropelException
     */
    public function getAssignments(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collAssignments || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collAssignments) {
                ...
                $collAssignments = ChildAssignmentQuery::create(null, $criteria)
    ...
    /**
     * Sets a collection of ChildAssignment objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $assignments A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildTeacher The current object (for fluent API support)
     */
    public function setAssignments(Collection $assignments, ConnectionInterface $con = null)
    {
        /** @var ChildAssignment[] $assignmentsToDelete */
        $assignmentsToDelete = $this->getAssignments(new Criteria(), $con)->diff($assignments);


        $this->assignmentsScheduledForDeletion = $assignmentsToDelete;
     ...
     * @throws PropelException
     */
    public function countAssignments(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collAssignmentsPartial && !$this->isNew();
        if (null === $this->collAssignments || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collAssignments) {
            ...
            $query = ChildAssignmentQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
    ...
    /**
     * Method called to associate a ChildAssignment object to this object
     * through the ChildAssignment foreign key attribute.
     *
     * @param  ChildAssignment $l ChildAssignment
     * @return $this|\API\Model\Teacher The current object (for fluent API support)
     */
    public function addAssignment(ChildAssignment $l)
    {
        if ($this->collAssignments === null) {
            $this->initAssignments();
            $this->collAssignmentsPartial = true;
        ...
        if (!$this->collAssignments->contains($l)) {
            $this->doAddAssignment($l);

            if ($this->assignmentsScheduledForDeletion and $this->assignmentsScheduledForDeletion->contains($l)) {
                $this->assignmentsScheduledForDeletion->remove($this->assignmentsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildAssignment $assignment The ChildAssignment object to add.
     */
    protected function doAddAssignment(ChildAssignment $assignment)
    {
    ...
    /**
     * @param  ChildAssignment $assignment The ChildAssignment object to remove.
     * @return $this|ChildTeacher The current object (for fluent API support)
     */
    public function removeAssignment(ChildAssignment $assignment)
    {
        if ($this->getAssignments()->contains($assignment)) {
            $pos = $this->collAssignments->search($assignment);
            $this->collAssignments->remove($pos);
